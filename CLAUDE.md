...
- **Database migration**: on `load`, if the `directories` table's stored root differs from the current root, `UPDATE ... SET col = ? || SUBSTR(col, LENGTH(?)+1)` rewrites the prefix in `track_locations` (location, directory), `LibraryHashes` (directory_path), and `directories` (directory) in place.
- **Config migration**: `mixxx.cfg`'s `Directory ...` line is rewritten the same way via a text scan/replace; if no `Directory` line exists yet, one is appended under `[Library]`.
