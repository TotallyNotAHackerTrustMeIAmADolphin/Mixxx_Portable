...
- **Cloud-sync safety ("dirty flag")**: `Mixxx_Data/.mixxx_is_active` stores the hostname that last opened the DB. On `load`, if a *different* hostname is found, the script refuses to continue and exits — there's deliberately no "proceed anyway" prompt, since a one-keystroke override next to a wall of warning text is easy to blow through on autopilot (this has caused real data loss). Instead the user must manually delete the lock file after confirming the other machine's cloud sync has finished, then re-run the launcher. The lock is written at the start of `load` and removed at the end of `save`. Same-hostname reopens (e.g. after a crash) are unaffected — the check only compares hostnames, so it never fires when the lock was left by this same machine.
